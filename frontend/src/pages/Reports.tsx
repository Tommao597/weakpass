...
export const Reports = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      // Only show completed or stopped tasks in reports
      const completedTasks = rawData.filter((t: any) => t.status === 'completed' || t.status === 'stopped');
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">查看历史检测结果并导出详细安全报告</p>
      </div>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                      <Clock size={10} /> {new Date(task.created_at || Date.now()).toLocaleDateString()}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-primary font-medium">
                      发现 {task.vulnerabilities_count || 0} 个弱口令
                    </span>
                  </div>
                </button>
              ))
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden animate-in fade-in duration-300">
              <div className="p-6 border-b border-border flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">检测完成于 {new Date().toLocaleString()}</p>
                </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] px-1.5 py-0.5 bg-muted rounded text-muted-foreground uppercase">{res.protocol}</span>
                            </div>
                            <p className="text-sm text-muted-foreground">
                              账号: <span className="text-foreground font-mono">{res.username}</span> | 
                              密码: <span className="text-red-500 font-mono">{res.password}</span>
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] text-muted-foreground block mb-1">检测时间</span>
