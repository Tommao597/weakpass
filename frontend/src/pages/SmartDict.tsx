...
export const SmartDict = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (!hasValue) {
      toast.error('请至少填写一项信息（如姓名、生日等）以生成字典');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        use_rule: formData.use_rule,
        use_ai: formData.use_ai,
        limit: formData.limit
      };

      const res = activeTab === 'smart'
        ? await smartDictApi.generate(payload)
        : await passwordApi.generateDict(payload);
