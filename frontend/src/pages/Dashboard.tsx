...
export const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const weakPasswords = tasks.reduce((acc: number, task: any) => acc + (task.vulnerabilities_count || 0), 0);
      
      // Calculate health based on task success rate
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">欢迎回来，这是您当前的安全检测概况</p>
        </div>
          ...
          <div className="divide-y divide-border">
            {stats.recentTasks.length === 0 ? (
              ...
              stats.recentTasks.map((task, i) => (
                <div key={i} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => navigate('/reports')}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{task.protocols?.join('/')} • {new Date(task.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className={`text-xs px-2 py-0.5 rounded ${
                      task.vulnerabilities_count > 0 ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'
                    }`}>
                      {task.vulnerabilities_count > 0 ? '发现风险' : '安全'}
                    </span>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <span className="font-bold">一键快速扫描</span>
            </button>
            <button 
              onClick={() => navigate('/dictionary')}
            ...
            >
              <FileText size={20} />
              <span className="font-bold">生成周报</span>
            </button>
          </div>
          ...
          <div className="pt-6 border-t border-border">
            <div className="bg-primary/10 border border-primary/20 rounded-xl p-4">
              ...
              <p className="text-sm text-foreground/80">建议定期更新您的常用弱口令字典，以保持检测的有效性。</p>
            </div>
          </div>
        </div>
      </div>
